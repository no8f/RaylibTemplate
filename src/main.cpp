...
int main(void)
{
	...
	for ( int i = 0; i < MAX_UNDO_STATES; i++ )
		memcpy(&states[i], &player, sizeof(PlayerState));
	...
	while ( !WindowShouldClose() ) // Detect window close button or ESC key
	{
		...
		if ( lastUndoIndex > firstUndoIndex ) {
			for ( int i = firstUndoIndex; i < currentUndoIndex; i++ )
				DrawRectangleRec(
						(Rectangle) {gridPosition.x + states[i].cell.x * GRID_CELL_SIZE, gridPosition.y + states[i].cell.y * GRID_CELL_SIZE,
			...
			if ( (currentUndoIndex < MAX_UNDO_STATES) && (currentUndoIndex > lastUndoIndex) ) {
				...
				for ( int i = firstUndoIndex; i < MAX_UNDO_STATES; i++ )
					DrawRectangle(
							(int) gridPosition.x + states[i].cell.x * GRID_CELL_SIZE, (int) gridPosition.y + states[i].cell.y * GRID_CELL_SIZE,
		...
		for ( int y = 0; y <= MAX_GRID_CELLS_Y; y++ )
			DrawLine(
					(int) gridPosition.x, (int) gridPosition.y + y * GRID_CELL_SIZE, (int) gridPosition.x + MAX_GRID_CELLS_X * GRID_CELL_SIZE,
					(int) gridPosition.y + y * GRID_CELL_SIZE, GRAY
			);
		for ( int x = 0; x <= MAX_GRID_CELLS_X; x++ )
			DrawLine(
					(int) gridPosition.x + x * GRID_CELL_SIZE, (int) gridPosition.y, (int) gridPosition.x + x * GRID_CELL_SIZE,
					(int) gridPosition.y + MAX_GRID_CELLS_Y * GRID_CELL_SIZE, GRAY
			);

		// Draw player
		DrawRectangle(
				(int) gridPosition.x + player.cell.x * GRID_CELL_SIZE, (int) gridPosition.y + player.cell.y * GRID_CELL_SIZE, GRID_CELL_SIZE + 1,
				GRID_CELL_SIZE + 1, player.color
		);
